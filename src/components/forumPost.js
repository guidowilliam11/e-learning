...
export default function ForumPost({ avatarSrc, author, date, content, tags, views, likes, comments }) {
  return (
    <div className="flex flex-grow bg-white rounded-lg shadow-md p-4 gap-5">
      <div className="w-full max-w-[180px] h-auto aspect-square bg-gray-200 rounded-md"></div>
      <div className='flex-grow flex flex-col justify-between'>
        ...
        <div className="flex space-x-2 mb-4">
          {tags.map((tag, index) => (
            <span key={index} className="text-xs bg-gray-200 text-gray-600 rounded-full px-3 py-1">
              {tag}
            </span>
          ))}
        </div>
        ...
        <div className='flex justify-between'>
          ...
          <div className="flex items-center gap-3">
            <Image src={avatarSrc} alt={`${author} avatar`} width={36} height={36} className="rounded-full h-[36px] w-[36px]" />
            <div>
